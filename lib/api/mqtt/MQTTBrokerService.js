...
function mqtt(){
  this.add('receiver:leaf,protocol:mqtt,cmd:publish', publish);

  function publish(msg, respond){

    let topic;
    if (!msg.portAddress) {
      topic = `/${msg.leafAddress}/${msg.type}`;
    } else {
      topic = `/${msg.leafAddress}/${msg.type}/${msg.portAddress}`;
    }

    let mqttMessage = {
      topic: topic,
      payload: JSON.stringify(msg.body), // or a Buffer
      qos: 0, // 0, 1, or 2
