...
function message(topic, message) {
  ...
  winston.debug("Incoming topic: " + topic);
  let parsedMessage = parseMessage(message);
  winston.debug(JSON.stringify(parsedMessage, null, '\t'));

  if (!parsedMessage) {
    winston.debug("Couldn't parse message");
    return;
  }
...
function publish(message, leafAddress, type, portAddress) {
  if (!client) {
    winston.error("MQTT Client not connected! Could not transmit message...");
    return;
  }
  if (!portAddress) {
...
function registerForTopic(topic, newListener, priority) {
  ...
  if (addListener) {
    ...
    winston.debug(`Listener registered for {topic = ${topic}} with {priority = ${priority}}.`);
  }
}
...
function parseMessage(message) {
  try {
    if (typeof message == 'string') {
      winston.debug('Message is a string');
      return JSON.parse(message);
    } else if (message instanceof Buffer) {
      winston.debug('Message is a Buffer');
      return JSON.parse(new Buffer(message).toString('utf-8'));
    } else if (message instanceof Object) {
      winston.debug('Message is an Object');
      return message;
    }
  } catch (err) {
  }
